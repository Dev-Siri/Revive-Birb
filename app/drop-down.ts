export default function dropDown() {
  ...
  const retweetOption = document.querySelector(
    "div[data-testid=retweetConfirm] > div:nth-child(2) > div > span"
  );
  const quoteOption = document.querySelector(
    "div[data-testid=Dropdown] > a[role=menuitem] > div:nth-child(2) > div > span"
  );

  if (retweetOption) retweetOption.textContent = "Retweet";
  if (quoteOption?.textContent === "Quote")
    quoteOption.textContent = "Quote Tweet";
